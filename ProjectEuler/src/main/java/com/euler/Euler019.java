...
public class Euler019 {
	...
	private final static int FIRST_DAY = 366 % 7; // Because we're counting from 1901, but the date we are given is from
													// 1900!!!
	...
	private static long count(int initialYear, int finalYear, int firstDay) {
		...
		long result = 0;
		for (int i = 0; i < 14; ++i)
			result += years[i].countFirstSundays() * counter[i];
		return result;
	}
